...
export function parseDepositTicket(text: string): ParsedDepositTicket | null {
  // Match value after label, stopping at next known label or newline
  // Handles both "orderId: 123\ntxid: 456" and "orderId: 123txid: 456" (no newlines)
  const orderIdMatch = text.match(/orderId\s*:\s*(.+?)(?=(?:txid|currency|amount)\s*:|[\n]|$)/i);
  const txidMatch = text.match(/txid\s*:\s*(.+?)(?=(?:orderId|currency|amount)\s*:|[\n]|$)/i);
  const currencyMatch = text.match(/currency\s*:\s*(.+?)(?=(?:orderId|txid|amount)\s*:|[\n]|$)/i);
  const amountMatch = text.match(/amount\s*:\s*(.+?)(?=(?:orderId|txid|currency)\s*:|[\n]|$)/i);
...
export function buildTicketLookupPrompt(ticket: ParsedDepositTicket): string {
  return [
    ...
    `Look up this deposit ticket using the TXID (payment_id) below.`,
    ``,
    `TXID (payment_id): ${ticket.txid}`,
    `Order ID (payment_customer_order_reference): ${ticket.orderId}`,
    `Currency: ${ticket.currency}`,
    `Amount: ${ticket.amount}`,
    ``,
    `IMPORTANT: Use the lookup_by_id tool with the TXID "${ticket.txid}" FIRST — this is the unique payment_id and will return the exact transaction.`,
    `Only if TXID not found, try the Order ID "${ticket.orderId}" as fallback.`,
