...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        try {
          ...
          // If embedding column doesn't exist or pgvector failed, retry without it
          ...
          if (dbMsg.includes("embedding") || dbMsg.includes("vector")) {
            await query(
              `INSERT INTO pascal_knowledge_base
               ...
               VALUES ($1, $2, $3, $4, $5, 'dashboard:bulk', 1.0, $6, 5, true)`,
              [entry.category, entry.match_pattern, entry.title, entry.content, entry.action || null, entry.business_id || null]
            );
          } else {
            throw dbErr;
          }
    ...
    console.error("Training bulk POST error:", err);
